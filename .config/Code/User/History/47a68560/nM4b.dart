  ...
  void _init() {
    ...
    file.parent.watch(events: FileSystemEvent.modify).listen((event) {
      if (event.path.endsWith('flutter_colors.json')) {
        ...
        Future.delayed(const Duration(milliseconds: 100), () => _updateColors(file));
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder<Map<String, Color>>(
      ...
      builder: (context, walColors, child) {
        ...
        final accent = walColors['color4'] ?? AppColors.accent; // color4 suele ser el acento en pywal
        ...
        return MaterialApp(
          ...
          theme: ThemeData(
            ...
            sliderTheme: SliderThemeData(
              ...
              inactiveTrackColor: (walColors['color8'] ?? const Color(0xFF1e2030)).withAlpha(150),
              thumbColor: text,
...
class _SpotifyWidgetState extends State<SpotifyWidget> {
  ...
  Future<String> _runPlayerCtl(List<String> args) async {
    try {
      final result = await Process.run('playerctl', ['--player=spotify', ...args]);
  ...
  void _updateInfo() async {
    ...
    if (mounted) {
      setState(() {
        ...
        if (lenStr.isNotEmpty) _duration = (double.tryParse(lenStr) ?? 0) / 1000000;
  ...
  Widget build(BuildContext context) {
    ...
    final accentColor = walColors['color4'] ?? AppColors.accent; // Color 4 suele ser azul/acento
    final textColor = walColors['foreground'] ?? AppColors.text;
    final secondaryTextColor = (walColors['color7'] ?? Colors.grey).withAlpha(200);

    return Scaffold(
      backgroundColor:bgColor.withAlpha(150), 
      body: Padding( 
          ...
          children: [
              ...
              children: [
                InkWell(
                  ...
                  child: Container(
                    ...
                    child: Icon(Icons.close_rounded, color: secondaryTextColor, size: 18),
                  ),
                )
              ],
            ),
            ...
            Container(
              ...
              child: _artUrl.isEmpty 
                  ? const Icon(Icons.music_note_rounded, color: Colors.white24, size: 80)
                  : null,
            ),
            ...
            Slider(
              ...
              onChanged: (val) {
                setState(() { _isSeeking = true; _position = val; });
              },
              onChangeEnd: (val) {
            ...
            Row(
              ...
              children: [
                Text(_formatTime(_position), style: TextStyle(color: secondaryTextColor, fontSize: 11)),
                Text(_formatTime(_duration), style: TextStyle(color: secondaryTextColor, fontSize: 11)),
              ],
            ),
            ...
            Row(
              ...
              children: [
                _buildBtn(
                  ...
                  size: 20,
                  onTap: () => _sendCommand(['shuffle', _isShuffle ? 'Off' : 'On'])
                ),
                _buildBtn(
              ...
              //     ),
                  ...
                  Card(
                    shape: BeveledRectangleBorder(borderRadius: BorderRadiusGeometry.circular(100)), 
                    color:  textColor.withAlpha(100),
                    child: InkWell(child:  Icon(_isPlaying ? Icons.pause_rounded : Icons.play_arrow_rounded,  size: 30,),
                    onTap: () => _sendCommand(['play-pause']),)),
                ...
                _buildBtn(
                  ...
                  size: 20,
                  onTap: () => _sendCommand(['loop', _loopStatus == "None" ? 'Playlist' : 'None'])
                ),
              ],
            ),
  ...
  Widget _buildBtn({required IconData icon, required Color color, required double size, required VoidCallback onTap}) {
    return IconButton(
